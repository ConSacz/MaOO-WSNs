...
# helper to get phi index (random of best 10%)
def get_phi_idx(Farr):
    top_k = max(1, int(np.ceil(0.1 * Farr.size)))
    choices = _rng.choice(np.argsort(Farr)[:top_k])
    choice_idx = _rng.integers(0, choices.shape)
...
def replace_parent_if_better(pop_X, pop_F, child_X, child_F, idx):
    if np.all(child_F <= pop_F[idx]) and np.any(child_F < pop_F[idx]):
...
while FES < MAXFES:
    ...
    op_children = []
    op_children_idx = []
    op_best_vals = np.full((3, n_obj), np.inf)
    ...
    for op in range(n_ops):
        ...
        for ii, pid in enumerate(idxs):
            ...
            if np.all(fu <= op_best_vals[op]) and np.any(fu < op_best_vals[op]):
                op_best_vals[op] = fu
                op_best_X[op, :] = u
    ...
    for op in range(n_ops):
        ...
        for k, pid in enumerate(idxs):
            ...
            FES += 1
            if np.all(fu <= Fvals[pid]) and np.any(fu < Fvals[pid]):
                ...
                if archive_X.shape[0] > archive_size:
                    ...
                    worst_idx = np.argmax(arch_f) // arch_f.shape[1]
        ...
        if members.size > 0:
            ...
            best_idx = members[np.argmin(Fvals[members]) // Fvals.shape[1]]
            # distances
    ...
    best_idx_overall = int(np.argmin(Fvals)//Fvals.shape[1])
    best_f = Fvals[best_idx_overall]
