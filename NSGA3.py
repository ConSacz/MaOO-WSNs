...
# ----------------------------
def das_dennis_generate(M, p):
    # M: objectives, p: divisions (positive integer)
    ...
    combos = recursive_gen(M, p, 0)
    W = np.array(combos, dtype=float) / float(p)
...
# ----------------------------
def nsga3(
    problem_eval,  # callable x -> f (array of M)
    D,             # decision variables dimension
    n_obj,         # number of objectives
    pop_size=100,
    max_gen=200,
    xmin=None,
    xmax=None,
    ref_divisions=5,  # Das & Dennis divisions (adjust for number of refs)
    sbx_eta=30,
    mut_eta=20,
    pm=None,
    seed=None,
    ...
    return X, F, ref_dirs, ideal

# ----------------------------
# Example: ZDT1 (2-objectives) for quick test
# ----------------------------
if __name__ == "__main__":
    ...
    D = 5
    X, F, ref_dirs, ideal = nsga3(dtlz2, D=D, n_obj=3, pop_size=100, max_gen=200, xmin=np.zeros(D), xmax=np.ones(D), ref_divisions=99, seed=1)
